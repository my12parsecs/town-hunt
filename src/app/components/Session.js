...
export function useSession() {
  ...
  const session = useQuery({
    queryKey: ['session'],
    queryFn: () => authFetch(`${process.env.NEXT_PUBLIC_BACKEND_DOMAIN}/session`),
    retry: false,
  ...
  const logout = useMutation({
    mutationFn: () => authFetch(`${process.env.NEXT_PUBLIC_BACKEND_DOMAIN}/logout`, { 
      method: 'POST' 
    }),
    onSuccess: () => {
      if (typeof window !== 'undefined') {
  ...
  return {
    ...
    authFetch // Export this so other components can make authenticated requests
  };
}